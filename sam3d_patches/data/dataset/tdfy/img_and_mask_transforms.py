...
class ObjectCentricSSI(SSIPointmapNormalizer):
    ...
    def _compute_scale_and_shift(self, pointmap: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        has_valid = mask_points.numel() > 0 and mask_points.isfinite().any()
        if not has_valid:
            if self.raise_on_no_valid_points:
                raise ValueError(f"No valid points found in mask")
            # Progressive dilation: expand mask until we find valid depth points
            import torch.nn.functional as _F
            dilated = mask_resized.unsqueeze(0).unsqueeze(0).float()
            found = False
            for ksize in [5, 11, 21, 41]:
                pad = ksize // 2
                dilated = _F.max_pool2d(dilated, kernel_size=ksize, stride=1, padding=pad)
                d_bool = dilated.squeeze().reshape(-1) > 0.5
                d_points = pointmap_flat[:, d_bool]
                if d_points.numel() > 0 and d_points.isfinite().any():
                    finite_d = d_points[:, d_points.isfinite().all(dim=0)]
                    if finite_d.shape[1] >= 10:
                        logger.info(f"Dilation ksize={ksize}: {finite_d.shape[1]} valid points found for scale calibration")
                        mask_points = finite_d
                        found = True
                        break
            if not found:
                finite_mask = pointmap_flat.isfinite().all(dim=0)
                if finite_mask.any():
                    logger.warning(f"No valid points in mask after dilation; falling back to full-image pointmap")
                    mask_points = pointmap_flat[:, finite_mask]
                else:
                    logger.warning(f"No valid points found anywhere; setting scale to {self.scale_factor} and shift to 0")
